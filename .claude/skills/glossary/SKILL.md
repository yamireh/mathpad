...
| **Undo** | Undo action | `ScratchToolbar.onUndo` (scratch area) · `AnswerPad` undo button (answer pad) · `ScratchCanvas.undo()` handle | Scratch undo removes the last scratch stroke. Answer/working-cell undo removes the **whole last box entry** in one step, then refocuses that box — `usePracticeSession` coalesces a cell's consecutive edits (all strokes of a digit + the live-recognition swap) into one undo step via `lastEditRef`/`coalesceKey`. |
...
| **Carrying boxes** (also "carry boxes") | **CarryBox** | `components/domain/CarryBox.tsx` | The small digit slot above a column for a carried digit. Used by addition/multiplication (carry row above the columns / times-carry above op1) and by **long division** (divisor-carry boxes above a multi-digit divisor for the active quotient step, ids `dcarry-{step}-{col}`, rendered in `components/domain/problem/DivisionProblem.tsx`). **No carry box is drawn above a leading answer column that only holds a final carry-out** (e.g. the `1` in `70 + 60 = 130`) — the kid writes that digit straight into the answer. Applies to **addition** (`additionCarries` + `CarryRow leadingSkip` in `AdditionProblem`) **and multiplication's final sum** (`multiplicationCarries` + `CarryRow leadingSkip` in `MultiplicationProblem`, e.g. the thousands `1` in `99 × 11 = 1089`). All in `workspace/multiOperand.ts`: the `leadingSkip` prop hides the box and the `*Carries` helper drops it from auto-advance. |
